/// <reference path="bookdevicebydetails.cy.js" />
import * as _ from '../../support/util';

describe('Device Booking', () => {
    ...
    beforeEach(function () {
        cy.visit('/Cases/CaseSelection')
    })

    it('Book Device by Device Booking Tab --cancel', () => {
        ...
        cy.get('[data-testid="caseListRow-1"]').then(() => {
            ...
            cy.get('[data-testid^="book-"]').click().then(() => {
                cy.url({ timeout: 5000 })
            })
        });
        // Add devices to book
    ...
    it('Book Device by Device Booking Tab --success', () => {
        // Search for test case
        cy.get('[data-testid="searchCase"]').click().then(() => {
